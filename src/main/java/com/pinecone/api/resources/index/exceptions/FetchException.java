...
@JsonDeserialize(
    using = FetchException.Deserializer.class
)
public final class FetchException extends Exception {
  ...
  private FetchException(Value value, int statusCode) {
    this.value = value;
  ...
  public static FetchException other(Object unknownValue, int statusCode) {
    return new FetchException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "FetchException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<FetchException> {
    @Override
    public FetchException deserialize(JsonParser p, DeserializationContext ctx) throws IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new FetchException(value, statusCode);
    }
  }
}
