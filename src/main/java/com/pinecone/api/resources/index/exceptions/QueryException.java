...
@JsonDeserialize(
    using = QueryException.Deserializer.class
)
public final class QueryException extends Exception {
  ...
  private QueryException(Value value, int statusCode) {
    this.value = value;
  ...
  public static QueryException other(Object unknownValue, int statusCode) {
    return new QueryException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "QueryException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<QueryException> {
    @Override
    public QueryException deserialize(JsonParser p, DeserializationContext ctx) throws IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new QueryException(value, statusCode);
    }
  }
}
