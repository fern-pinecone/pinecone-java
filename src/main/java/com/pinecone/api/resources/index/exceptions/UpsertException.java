...
@JsonDeserialize(
    using = UpsertException.Deserializer.class
)
public final class UpsertException extends Exception {
  private final Value value;

  private int statusCode;

  private UpsertException(Value value, int statusCode) {
    this.value = value;
  ...
  public static UpsertException other(Object unknownValue, int statusCode) {
    return new UpsertException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "UpsertException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<UpsertException> {
    @Override
    public UpsertException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new UpsertException(value, statusCode);
    }
  }
}
